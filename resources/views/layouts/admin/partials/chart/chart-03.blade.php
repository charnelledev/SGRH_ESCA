  ...
  <div class="flex flex-col gap-5 mb-6 sm:flex-row sm:justify-between">
    <div class="w-full">
      ...
      <p class="mt-1 text-gray-500 text-theme-sm dark:text-gray-400">
        Suivi mensuel des employés, emplois et régions
      </p>
    </div>

    <div class="flex items-start w-full gap-3 sm:justify-end">
      <div
        x-data="{selected: 'vue_generale'}"
        class="inline-flex w-fit items-center gap-0.5 rounded-lg bg-gray-100 p-0.5 dark:bg-gray-900"
      >
        <button
          @click="selected = 'vue_generale'"
          :class="selected === 'vue_generale' ? 'shadow-theme-xs text-gray-900 dark:text-white bg-white dark:bg-gray-800' : 'text-gray-500 dark:text-gray-400'"
          class="px-3 py-2 font-medium rounded-md text-theme-sm hover:text-gray-900 dark:hover:text-white"
        >
          Vue générale
        </button>
        <button
          @click="selected = 'contrats'"
          :class="selected === 'contrats' ? 'shadow-theme-xs text-gray-900 dark:text-white bg-white dark:bg-gray-800' : 'text-gray-500 dark:text-gray-400'"
          class="px-3 py-2 font-medium rounded-md text-theme-sm hover:text-gray-900 dark:hover:text-white"
        >
          Contrats
        </button>
        <button
          @click="selected = 'salaires'"
          :class="selected === 'salaires' ? 'shadow-theme-xs text-gray-900 dark:text-white bg-white dark:bg-gray-800' : 'text-gray-500 dark:text-gray-400'"
          class="px-3 py-2 font-medium rounded-md text-theme-sm hover:text-gray-900 dark:hover:text-white"
        >
          Salaires
        </button>
